...
// TODO: This size just happened to test the case where we write all the way up to one byte before
// the end of the store.  This caused an issue because we would see that the store potentially had
// more data, and then follow a codepath that tried to read a uint32_t value to get the size of the
// next block.
